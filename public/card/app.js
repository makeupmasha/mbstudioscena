'use strict';
(() => {
  const CARD_URL = document.querySelector('#card-url').value;
  const published = document.body.dataset.preview !== '1' && location.protocol === 'https:' && location.origin === new URL(CARD_URL).origin && /^\/card\/(?:index\.html)?$/.test(location.pathname);
