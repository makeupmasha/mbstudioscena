...
from scena_tenant import profile_defaults, DEMO_SERVICES, SERVICE_EN
DEFAULT_SETTINGS.update(profile_defaults())
...
DEFAULT_SERVICES = DEMO_SERVICES
...
def init_db(db_path: str | Path, *, now: datetime | None = None) -> None:
    ...
    with _connect(db_path) as connection:
        ...
        for service_name, (name_en, description_en) in SERVICE_EN.items():
            connection.execute("UPDATE services SET name_en=?,description_en=? WHERE name=? AND name_en='' AND description_en=''", (name_en, description_en, service_name))

        from scena_tenant import seed_publications
        seed_publications(connection)
