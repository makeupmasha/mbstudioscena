...
def navigation(locale, section, view, sections, views, owner):
    ...
    with st.container(key='scena_cabinet_chrome'):
        st.markdown('<header class="scena-cabinet-header">'+'<a class="scene-home-brand" aria-label="Sofi Leroux. SCENA.live" data-cabinet-nav href="/?'+urlencode(dict(page='scene',lang=locale))+'">'+scene_brand_markup()+'</a>'+
