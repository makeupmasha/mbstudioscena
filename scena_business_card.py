...
from scena_tenant import public_origin, BRAND, is_demo
...
DEFAULT_CARD = {
    ...
    'primary_url': public_origin() + '/ru/zapis/',
    'site_url': public_origin() + '/ru/',
    'instagram_url': '',
    'telegram_url': '',
    'phone': '',
    'email': '',
...
def validate_card(values):
    ...
    for key in ('primary_url', 'site_url', 'instagram_url', 'telegram_url'):
        ...
        try:
            url = urlsplit(value)
            local_preview = public_origin().startswith('http://localhost:') and value.startswith(public_origin() + '/')
            valid = bool(local_preview or (url.scheme == 'https' and url.hostname and not url.username and not url.password and url.port in (None, 443)))
        except ValueError:
            valid = False
...
def vcard(card):
    ...
    lines = ['BEGIN:VCARD', 'VERSION:3.0', 'N:' + escape(card['last_name']) + ';' + escape(card['first_name']) + ';;;',
             'FN:' + escape(full_name(card)), 'ORG:' + BRAND]
...
def render_card(card, *, preview=False, photo_url=None):
    ...
    return _template().generate(
        ...
        card_url=CARD_URL, public_origin=public_origin(), photo_url=photo_url, preview=preview, demo=is_demo(),
        phone_href='tel:' + re.sub(r'[^+0-9]', '', card['phone']),
        email_href='mailto:' + quote(card['email'], safe='@.'),
        contact_url=('data:text/vcard;charset=utf-8;base64,' + base64.b64encode(vcard(card)).decode()) if preview else '/card/contact.vcf',
    ).decode('utf-8')
