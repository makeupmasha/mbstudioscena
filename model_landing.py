...
<main class="shell{" intro-open" if intro else ""}" id="top" data-view="{"intro" if intro else "images"}">
  <section class="stage" aria-roledescription="carousel" aria-label="SCENA — {html.escape(master_name, quote=True)}">
    ...
    <header class="header">
      <a class="wordmark scene-home-brand" target="{navigation_target}" rel="noopener noreferrer" href="{html.escape(scene_url, quote=True)}" aria-label="Sofi Leroux. SCENA.live">{scene_brand_markup(light=True)}</a>
