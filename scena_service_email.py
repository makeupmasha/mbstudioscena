...
The sender comes from this installation's verified provider configuration.
Request Host headers never choose the sender address.
"""
...
MAIL_ROOT_DOMAIN = os.environ.get('SCENA_MAIL_ROOT_DOMAIN', '').strip().lower()
RESEND_ENDPOINT = "https://api.resend.com/emails"
EVENTS = ("received", "confirmed")


def sender_from_public_base(public_base_url: str) -> str:
    """Use a separately verified sender, or a tenant under our configured domain."""
    explicit = os.environ.get('SCENA_EMAIL_FROM', '').strip()
    if explicit:
        return explicit if re.fullmatch(r"[A-Za-z0-9.!#$%&'*+/=?^_`{|}~-]+@[A-Za-z0-9](?:[A-Za-z0-9.-]*[A-Za-z0-9])?\.[A-Za-z]{2,63}", explicit) else ''
    if not MAIL_ROOT_DOMAIN:
        return ''
    try:
