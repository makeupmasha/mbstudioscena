...
class CardContent(RequestHandler):
    ...
    async def get(self, resource=None):
        ...
        if resource == 'contact.vcf':
            self.set_header('Content-Type', 'text/vcard; charset=utf-8')
            self.set_header('Content-Disposition', 'attachment; filename="Sofi-Leroux.vcf"')
...
class CardQr(RequestHandler):
    def get(self):
        ...
        output = io.BytesIO()
        qrcode.make(public_origin() + '/card/', image_factory=SvgPathImage, border=2).save(output)
        public_headers(self)
...
def card_routes(*, db_path=None, app_dir=None):
    return [
        (r'/card', CardRedirect),
        (r'/card/qr\.svg', CardQr),
        (r'/card/(index\.html|contact\.vcf|portrait\.webp)?', CardContent,
