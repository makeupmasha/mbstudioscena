...
def initialize():
    ...
    with _lock:
        if _ready:
            return
        from .bootstrap_lock import database_boot_lock
...
def _initialize():
    ...
    # This is a new installation, not a migration of the previous owner's domain.
    REPORT['tenant']='sofia-demo'
    from scena_tenant import public_origin
    with connect(database) as db:
        origin = db.execute("SELECT value FROM profile_settings WHERE key='public_base_url'").fetchone()
        if origin and str(origin[0]).startswith(('http://localhost:', 'http://127.0.0.1:')) and public_origin().startswith('https://'):
            db.execute("UPDATE profile_settings SET value=? WHERE key='public_base_url'", (public_origin(),))
